...
def compute_basics(hdf5datapath, overwrite=False, verbose=False, nu=1.004):
    def get_basics(udata, dx=None, dy=None):
        ...
        for i, epsilon in enumerate(epsilon_test):
            ...
            keta_test[i, :] = keta[:, 0]
        epsilon_sij = vel.get_epsilon_using_sij(udata, dx=dx, dy=dy, nu=nu)
...
def ask_what_to_plot(hdf5datapath):
    ...
    with h5py.File(hdf5datapath, 'a') as fyle:
        ...
        while True:
            try:
                n_piv_to_use = input('Enter number of pivdata to use: ')
        ...
        if n_piv_to_use == n_piv:
            ...
            while len(indices) < n_piv_to_use:
                try:
                    index = input('Choose which piv_data to use. Type integers. piv___ : ')
...
def get_energy_spectrum_1d_int(hdf5datapath, indices, t0=0):
        ...
        with h5py.File(hdf5datapath, 'a') as fyle:
            ...
            if event.inaxes is not None:
                ...
                if ax in axes_energy:
                    ...
                    print zip(xcoords, ycoords), button[event.button]
                ...
                # Show piv params IF any ax in axes_e_time_avg is clicked
                    ...
                    if button[event.button] == 'left':
                        pivparamnames = pivdata[indices[ax_ind]].attrs.keys()
                        pivparamvalues = pivdata[indices[ax_ind]].attrs.values()
                        text0 = str(dict(zip(pivparamnames, pivparamvalues)))
                        words = text0.split(', ')
                        text_list = []
...
def get_time_evolution(hdf5datapath, indices, t0=0):
    """
...
def compute_autocorrelations(hdf5datapath, coarse=1.0, coarse2=1.0, overwrite=False, verbose=True):
    ...
    def get_autocorrelations(udata_t, xx, yy, time, coarse=1.0, coarse2=0.2):
        ...
        Rij, autocorrs = vel.get_two_point_vel_corr_iso(udata_t, xx, yy, time=time, n_bins=None, coarse=coarse, coarse2=coarse2, notebook=False)
...
def compute_lengthscales(hdf5datapath, indices, t0=0, overwrite=False, verbose=True):
...
def get_lengthscales(hdf5datapath, indices=[], t0=0):
    print '-------------Length Scales------------------'
    with h5py.File(hdf5datapath, 'a') as fyle:
        n_piv = len(fyle['piv'])  # number of piv data for a given cine
        n_piv_to_use = len(indices)
...
def main(hdf5datapath, t=0, overwrite=False, verbose=False):
    ...
    get_energy_spectrum_1d_int(hdf5datapath, indices, t0=t)

    # TIME EVOLUTION
    get_time_evolution(hdf5datapath, indices=indices)


    # Computationally expensive task 1: Autocorrelation functions
    print overwrite
    rij, ff, gg = compute_autocorrelations(hdf5datapath, coarse=1.0, coarse2=0.2,  overwrite=overwrite)


    # LENGTHSCALES AND REYNOLDS NUMBERS
    ## TIME-EVOLUTION OF LENGTHSCALES AND REYNOLDS NUMBERS
    print overwrite
    compute_lengthscales(hdf5datapath, indices, t0=0, overwrite=overwrite, verbose=True)
    get_lengthscales(hdf5datapath, indices)
...
if __name__ == "__main__":
    ...
    main(args.h5data, overwrite=args.overwrite)
